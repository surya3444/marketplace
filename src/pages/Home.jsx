      ...
      <Link to={`/product/${product.id}`} className="h-64 relative bg-white dark:bg-white/5 block group/slider">
        ...
        {hasMultipleImages && (
            <>
              <button 
                onClick={prevImage} 
                className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/30 hover:bg-black/60 text-white rounded-full flex items-center justify-center opacity-0 group-hover/slider:opacity-100 transition z-10"
              >
              ...
              <button 
                onClick={nextImage} 
                className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/30 hover:bg-black/60 text-white rounded-full flex items-center justify-center opacity-0 group-hover/slider:opacity-100 transition z-10"
              >
...
const Home = () => {
    ...
    return (
      <div className="bg-pearl dark:bg-dark min-h-screen transition-colors duration-300 pt-20 pb-20">
          ...
          {HERO_SLIDES.map((slide, index) => (
              <div key={index} className={`absolute inset-0 transition-opacity duration-1000 ${index === currentHeroSlide ? 'opacity-100' : 'opacity-0'}`}>
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4 z-10">
                      ...
                      <p className="text-gray-200 text-lg mb-8 max-w-2xl drop-shadow-md animate-fade-in-up delay-100">{slide.sub}</p>
        ...
        {/* 3. PRODUCT GRID with CARD SLIDERS */}
